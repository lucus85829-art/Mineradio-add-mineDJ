...
const RADIO_AUDIO_DIR = path.join(__dirname, 'public', 'radio-audio');
const RADIO_MEMORY_FILE = path.join(__dirname, 'radio_memory.json');
const DEEPSEEK_API_KEY = process.env.DEEPSEEK_API_KEY || 'sk-5f68227274e54e6980c182e9d12b890e';
const DEEPSEEK_API_URL = 'https://api.deepseek.com/v1/chat/completions';
const RADIO_TTS_VOICE = 'zh-CN-XiaoxiaoNeural';
const RADIO_MAX_AUDIO_FILES = 30;
// Resolve edge-tts path at startup to avoid PATH-dependent spawn failures
var RADIO_EDGE_TTS_PATH = 'edge-tts';
(function findEdgeTts() {
  var cp = require('child_process');
  var paths = [];
  try {
    var r = cp.execSync('where edge-tts', { encoding: 'utf-8', timeout: 5000, stdio: ['ignore','pipe','ignore'] });
    paths = r.trim().split(/\r?\n/);
  } catch (e) {}
  if (!paths[0]) {
    try {
      var r = cp.execSync('which edge-tts', { encoding: 'utf-8', timeout: 5000, stdio: ['ignore','pipe','ignore'] });
      paths = r.trim().split(/\r?\n/);
    } catch (e) {}
  }
  if (paths[0] && paths[0].trim()) {
    RADIO_EDGE_TTS_PATH = paths[0].trim();
    console.log('[Radio] Found edge-tts at:', RADIO_EDGE_TTS_PATH);
  }
})();
(function ensureRadioAudioDir() {
  try { fs.mkdirSync(RADIO_AUDIO_DIR, { recursive: true }); } catch (e) {}
})();

// ---------- Radio persistent memory ----------
function loadRadioMemory() {
  try {
    if (fs.existsSync(RADIO_MEMORY_FILE)) {
      return JSON.parse(fs.readFileSync(RADIO_MEMORY_FILE, 'utf-8'));
    }
  } catch (e) { console.error('[Radio Mem] Load error:', e.message); }
  return { recommendedSongs: [], memorySummary: '', userTaste: '', updatedAt: 0 };
}
function saveRadioMemory(mem) {
  try {
    mem.updatedAt = Date.now();
    fs.writeFileSync(RADIO_MEMORY_FILE, JSON.stringify(mem, null, 2), 'utf-8');
  } catch (e) { console.error('[Radio Mem] Save error:', e.message); }
}
function updateRadioMemorySong(song) {
  var mem = loadRadioMemory();
  var key = (song.title || song.name || '') + ' - ' + (song.artist || '');
  if (!key.trim()) return;
  // Avoid duplicates; keep max 200 entries
  mem.recommendedSongs = mem.recommendedSongs.filter(function(s) { return s.key !== key; });
  mem.recommendedSongs.unshift({
    key: key,
    title: song.title || song.name || '',
    artist: song.artist || '',
    recommendedAt: Date.now(),
    action: song.action || 'recommended',
  });
  if (mem.recommendedSongs.length > 200) mem.recommendedSongs = mem.recommendedSongs.slice(0, 200);
  saveRadioMemory(mem);
}
function formatRadioMemoryForPrompt() {
  var mem = loadRadioMemory();
  var block = '';
  if (mem.recommendedSongs.length > 0) {
    var recent = mem.recommendedSongs.slice(0, 30);
    block += '【AI已推荐过的歌曲（尽量避免重复推荐）】\n';
    recent.forEach(function(s, i) {
      block += (i + 1) + '. ' + s.title + ' - ' + s.artist + '\n';
    });
  }
  if (mem.memorySummary) {
    block += '\n【历史对话记忆】\n' + mem.memorySummary + '\n';
  }
  return block;
}
...
const server = http.createServer(async (req, res) => {
  ...
  // ---------- 随心电台 helper ----------
  var radioTtsLock = false;
  var radioTtsQueue = [];
  var radioTtsCache = new Map();  // hash -> { filePath, mtime }

  function buildRadioDjSystemPrompt(opts) {
    opts = opts || {};
    var listenSummary = opts.listenSummary || null;
    var playedSongs = opts.playedSongs || [];
    var songFeedback = opts.songFeedback || {};
    var isFirstVisit = opts.isFirstVisit === true;
    var memorySummary = opts.memorySummary || '';

    var hour = new Date().getHours();
    var timeOfDay = hour < 6 ? '凌晨' : hour < 9 ? '清晨' : hour < 12 ? '上午' : hour < 14 ? '中午' : hour < 18 ? '下午' : hour < 21 ? '傍晚' : '深夜';
    var season = (new Date().getMonth() + 1) >= 6 && (new Date().getMonth() + 1) <= 8 ? '夏天' : (new Date().getMonth() + 1) >= 3 && (new Date().getMonth() + 1) <= 5 ? '春天' : (new Date().getMonth() + 1) >= 9 && (new Date().getMonth() + 1) <= 11 ? '秋天' : '冬天';

    var statsBlock = '';
    if (listenSummary) {
      statsBlock = '【用户音乐画像】\n'
        + '总播放: ' + (listenSummary.totalPlays || 0) + ' 首，完整听完率: ' + (listenSummary.totalCompletes && listenSummary.totalPlays ? Math.round(listenSummary.totalCompletes / Math.max(1, listenSummary.totalPlays) * 100) + '%' : '新用户') + '\n';
      // Top artists
      var topA = listenSummary.topArtists5 || [];
      if (topA.length > 0) {
        statsBlock += '\n最常听的艺人（越靠前越喜欢）：\n';
        topA.forEach(function(a, i) {
          statsBlock += '  ' + (i + 1) + '. ' + a.name + ' (播放' + a.plays + '次)' + (a.listenMs > 0 ? ' 累计收听' + Math.round(a.listenMs / 60000) + '分钟' : '') + '\n';
        });
      }
      // Top songs
      var topS = listenSummary.topSongs5 || [];
      if (topS.length > 0) {
        statsBlock += '\n最常听的歌曲（越靠前越喜欢）：\n';
        topS.forEach(function(s, i) {
          var compRate = s.plays > 0 ? Math.round(s.completed / Math.max(1, s.plays) * 100) : 0;
          statsBlock += '  ' + (i + 1) + '. ' + s.name + ' - ' + s.artist + ' (播' + s.plays + '次，完播率' + compRate + '%)\n';
        });
      }
      // Recent plays
      var recent10 = listenSummary.recent10 || [];
      if (recent10.length > 0) {
        statsBlock += '\n最近听过的歌（按时间倒序，优先推荐风格相近的歌）：\n';
        recent10.slice(0, 8).forEach(function(r, i) {
          statsBlock += '  ' + (i + 1) + '. ' + (r.title || r.name || '?') + ' - ' + (r.artist || '?') + (r.completed ? '' : ' [提前切歌]') + '\n';
        });
      }
      // Backward compat: keep old fields for fallback
      if (!topA.length && listenSummary.topArtist) {
        statsBlock += '- 最常听的艺人: ' + listenSummary.topArtist.name + ' (播放' + (listenSummary.topArtist.plays || 0) + '次)\n';
      }
      if (!topS.length && listenSummary.topSong) {
        statsBlock += '- 最常听的歌曲: ' + listenSummary.topSong.title + ' (播放' + (listenSummary.topSong.plays || 0) + '次)\n';
      }
    }

    // B: Session play history
    var playedBlock = '';
    if (playedSongs.length > 0) {
      var recentPlays = playedSongs.slice(-15);
      playedBlock = '本次会话已播放的歌曲（按时间顺序）：\n';
      recentPlays.forEach(function(s, i) {
        playedBlock += (i + 1) + '. ' + (s.title || '未知') + ' - ' + (s.artist || '未知');
        if (s.action === 'skipped') playedBlock += ' [用户切歌]';
        if (s.action === 'liked') playedBlock += ' [用户听完]';
        playedBlock += '\n';
      });
      playedBlock += '（避免推荐以上已播放过的歌曲）\n';
    }

    // C: User feedback signals
    var feedbackBlock = '';
    var feedbackKeys = Object.keys(songFeedback);
    if (feedbackKeys.length > 0) {
      feedbackBlock = '用户偏好信号（根据行为推断）：\n';
      feedbackKeys.forEach(function(key) {
        var fb = songFeedback[key];
        if (fb.skipped) feedbackBlock += '- 切掉了: ' + key + '（可能不喜欢这类风格）\n';
        if (fb.liked) feedbackBlock += '- 完整听完: ' + key + '（应该喜欢这首歌）\n';
        if (fb.chatAbout > 0) feedbackBlock += '- 主动聊过: ' + key + '（对这首歌/艺人感兴趣）\n';
      });
      feedbackBlock += '请根据这些信号调整推荐方向：用户听完的歌多推荐类似风格，用户切掉的歌避开同类。\n';
    }

    // D: Memory summary from older conversation
    var memoryBlock = '';
    if (memorySummary) {
      memoryBlock = '【对话记忆摘要 — 之前你和用户聊过的要点】\n' + memorySummary + '\n';
    }
    var persistentMemBlock2 = formatRadioMemoryForPrompt();

    return {
      role: 'system',
      content: '你是一个名叫"mineDJ"的中文电台DJ。你性格温暖、风趣、有品味，说话像一个真实的夜间电台主持人。\n\n'
        + '当前上下文：\n'
        + '- 时间: ' + timeOfDay + '\n'
        + '- 季节: ' + season + '\n'
        + statsBlock + '\n'
        ...
        + persistentMemBlock2 + '\n'
        + '要求：\n'
        + '1. 回复必须是JSON格式：{"text": "你的DJ台词...", "mood": "relaxed|energetic|nostalgic|curious"}\n'
        + '2. DJ台词控制在15-30秒的朗读长度（约30-60个汉字），保持口语化自然\n'
        + '3. 可以偶尔提到用户的播放数据，但不要念数据，而是像朋友聊天一样自然地带过\n'
        + '4. 根据时间和用户口味选择合适的mood\n'
        + '5. 不要使用任何Markdown格式，纯文本即可\n'
        + '6. 不要用"mineDJ说"或类似的前缀，直接输出台词\n'
        + '7. 推荐歌曲时必须避开【已播放列表】中的歌，优先推荐和用户听完的歌风格相似的音乐\n'
        + '8. 台词中每个句子之间用换行符分隔（\\n），方便逐句朗读和显示。每个短句一行，不要一句话太长。',
    };
  }

  function buildRadioDjProgramPrompt(opts) {
    opts = opts || {};
    var listenSummary = opts.listenSummary || null;
    var playedSongs = opts.playedSongs || [];
    var songFeedback = opts.songFeedback || {};
    var isFirstVisit = opts.isFirstVisit === true;
    var memorySummary = opts.memorySummary || '';
    var recentSongs = (opts.recentSongs || []).slice(0, 8);
    ...
    var statsBlock = '';
    if (listenSummary) {
      statsBlock = '【用户音乐画像 -- 仔细研究这些数据，推荐必须高度匹配用户口味】\n'
        + '总播放: ' + (listenSummary.totalPlays || 0) + ' 首';
      if (listenSummary.totalCompletes && listenSummary.totalPlays) {
        statsBlock += '，完播率: ' + Math.round(listenSummary.totalCompletes / Math.max(1, listenSummary.totalPlays) * 100) + '%';
      }
      statsBlock += '\n';
      // Top 8 artists with full stats
      var topA = listenSummary.topArtists5 || [];
      if (topA.length > 0) {
        statsBlock += '\n▼ 最常听的艺人（TOP' + Math.min(topA.length, 8) + '，越靠前越喜欢，优先推荐这些艺人的歌曲）：\n';
        topA.slice(0, 8).forEach(function(a, i) {
          statsBlock += '  ' + (i + 1) + '. ' + a.name + ' (播放' + a.plays + '次' + (a.listenMs > 0 ? '，累计' + Math.round(a.listenMs / 60000) + '分钟' : '') + ')\n';
        });
      } else if (listenSummary.topArtist) {
        statsBlock += '最常听的艺人: ' + listenSummary.topArtist.name + ' (播放' + (listenSummary.topArtist.plays || 0) + '次)\n';
      }
      // Top 5 songs with completion rate
      var topS = listenSummary.topSongs5 || [];
      if (topS.length > 0) {
        statsBlock += '\n▼ 最常听的歌曲（TOP' + Math.min(topS.length, 5) + '，参考风格，但不要重复推荐这些具体歌曲）：\n';
        topS.slice(0, 5).forEach(function(s, i) {
          var compRate = s.plays > 0 ? Math.round((s.completed || 0) / Math.max(1, s.plays) * 100) : 0;
          statsBlock += '  ' + (i + 1) + '. ' + s.name + ' - ' + s.artist + ' (播' + s.plays + '次，完播率' + compRate + '%)\n';
        });
      } else if (listenSummary.topSong) {
        statsBlock += '最常听的歌曲: ' + listenSummary.topSong.title + ' (播放' + (listenSummary.topSong.plays || 0) + '次)\n';
      }
      // Recent 8 plays
      var recent10 = listenSummary.recent10 || [];
      if (recent10.length > 0) {
        statsBlock += '\n▼ 最近播放记录（时间倒序，标记[切歌]的表示用户不喜欢，应避开相似风格）：\n';
        recent10.slice(0, 8).forEach(function(r, i) {
          var flags = [];
          if (r.completed === true) flags.push('听完');
          if (r.completed === false) flags.push('切歌');
          statsBlock += '  ' + (i + 1) + '. ' + (r.title || r.name || '?') + ' - ' + (r.artist || '?') + (flags.length ? ' [' + flags.join(',') + ']' : '') + '\n';
        });
      } else if (listenSummary.recent) {
        statsBlock += '最近播放: ' + (listenSummary.recent.title || listenSummary.recent.name || '') + '\n';
      }
      // Usage hint
      statsBlock += '\n▲ 根据以上画像选歌：优先推荐TOP艺人的其他歌曲，风格对齐TOP歌曲，避开标记[切歌]的风格。\n';
    }

    var playedBlock = '';
    if (playedSongs.length > 0) {
      var recentPlays = playedSongs.slice(-15);
      playedBlock = '本次会话已播放歌曲（禁止重复推荐）：\n';
      recentPlays.forEach(function(s, i) {
        playedBlock += (i + 1) + '. ' + (s.title || '未知') + ' - ' + (s.artist || '未知') + '\n';
      });
    }
    ...
    var feedbackKeys = Object.keys(songFeedback);
    if (feedbackKeys.length > 0) {
      feedbackBlock = '用户偏好信号：\n';
      feedbackKeys.forEach(function(key) {
        var fb = songFeedback[key];
        if (fb.liked) feedbackBlock += '- 喜欢: ' + key + '\n';
        if (fb.skipped) feedbackBlock += '- 切掉: ' + key + '\n';
      });
    }

    var memoryBlock = '';
    if (memorySummary) { memoryBlock = '【对话记忆】\n' + memorySummary + '\n'; }
    // Inject persistent memory (cross-session)
    var persistentMemBlock = formatRadioMemoryForPrompt();

    var recentBlock = '';
    if (recentSongs.length > 0) {
      recentBlock = '最近播放的歌（参考风格，但不要重复推荐）：\n';
      recentSongs.forEach(function(s, i) {
        recentBlock += (i + 1) + '. ' + (s.title || s.name || '未知') + ' - ' + (s.artist || '未知') + '\n';
      });
    }

    return {
      role: 'system',
      content: '你是一个名叫\"mineDJ\"的中文电台DJ。你性格温暖、风趣、有品味，说话像一个真实的夜间电台主持人。你是电台节目的编排者，而不是单曲推荐者。\n\n' +
        '当前上下文：\n' +
        '- 时间: ' + timeOfDay + '\n' +
        '- 季节: ' + season + '\n' +
        statsBlock + '\n' +
        ...
        recentBlock + '\n' +
        persistentMemBlock + '\n' +
        '你的任务是编排一期完整的电台节目。你必须返回严格的JSON格式：\n' +
        '{\n' +
        '  "title": "本期节目名称（5-10字中文，像真实的电台节目名）",\n' +
        '  "mood": "relaxed|energetic|nostalgic|curious",\n' +
        '  "tracks": [\n' +
        '    {"song": "歌曲名", "artist": "歌手名"},\n' +
        '    {"song": "歌曲名", "artist": "歌手名"}\n' +
        '  ],\n' +
        '  "segments": [\n' +
        '    {"type": "cold_open", "text": "开场白，15-25秒"},\n' +
        '    {"type": "bridge", "afterTrackIndex": 0, "text": "过渡词，12-18秒"},\n' +
        '    {"type": "bridge", "afterTrackIndex": 1, "text": "过渡词"}\n' +
        '  ]\n' +
        '}\n\n' +
        '编排规则：\n' +
        '1. 每期推荐2-3首歌，必须有明确的节目主题将它们串联起来\n' +
        '2. 歌曲推荐必须避开【已播放列表】中的歌，优先推荐和用户最常听的艺人/歌曲风格相近的音乐\n' +
        '2b. 在用户最常听的艺人中选择不同的歌曲，或在用户喜欢的风格中推荐新歌，确保推荐理由和用户的音乐画像吻合\n' +
        '3. cold_open必须存在，bridge可以部分省略或用{"type":"silence","afterTrackIndex":N}表示故意不说话让歌曲连续播放\n' +
        '4. afterTrackIndex从0开始，表示在第几首歌之后\n' +
        '5. DJ台词保持口语化自然，每个短句不超过一行。不要用\"mineDJ说\"之类的前缀\n' +
        '6. 只输出JSON，不要任何额外文字或Markdown\n' +
        '7. 开场白中不要直接说\"我为你选了两首歌\"之类暴露歌曲数量的话，而是用节目主题和情绪来引入\n' +
        '8. ' + (isFirstVisit ? '用户在收听你的节目时第一次听到你的声音。请在cold_open开场白中简短介绍自己，告诉听众你叫什么名字（例如"我是mineDJ"）。' : '用户已经多次收听你的节目了，不要在cold_open中说"我是mineDJ"之类的自我介绍，直接聊今天的音乐主题。'),
    };
  }

  // Batch resolve track names to playable songs via Netease search
  async function resolveRadioTracks(tracks) {
    var resolved = [];
    var failed = [];
    for (var i = 0; i < tracks.length; i++) {
      var t = tracks[i];
      var songName = t.song || t.name || '';
      var artistName = t.artist || '';
      if (!songName) { failed.push({ song: songName, artist: artistName, reason: 'empty name' }); continue; }
      try {
        var query = songName + (artistName ? ' ' + artistName : '');
        var songs = await handleSearch(query, 3);
        if (songs.length > 0) {
          var song = songs[0];
          var loginInfo = await getLoginInfo();
          var urlInfo;
          try { urlInfo = await handleSongUrl(song.id, loginInfo, ''); } catch (e) { urlInfo = {}; }
          resolved.push({
            song: songName, artist: artistName,
            id: song.id, name: song.name || song.title || songName,
            title: song.title || song.name || songName,
            artist: song.artist || song.artists || artistName,
            cover: song.cover || '',
            url: urlInfo.url || '',
            playable: urlInfo.playable !== false && !!urlInfo.url,
          });
        } else {
          failed.push({ song: songName, artist: artistName, reason: 'no results' });
        }
      } catch (e) {
        failed.push({ song: songName, artist: artistName, reason: e.message });
      }
    }
    return { resolved: resolved, failed: failed };
  }

  // Batch synthesize TTS for all radio segments
  async function synthesizeRadioSegments(segments, voice) {
    voice = voice || RADIO_TTS_VOICE;
    for (var i = 0; i < segments.length; i++) {
      var seg = segments[i];
      if (!seg.text || seg.type === 'silence') {
        seg.audioUrl = '';
        seg.status = 'silent';
        continue;
      }
      try {
        var fname = 'djprog_' + Date.now() + '_' + i + '_' + Math.random().toString(36).slice(2, 6) + '.mp3';
        var outPath = path.join(RADIO_AUDIO_DIR, fname);
        await generateRadioTtsWithCache(seg.text, outPath, voice);
        seg.audioUrl = '/radio-audio/' + path.basename(outPath);
        seg.status = 'ready';
      } catch (e) {
        console.error('[Radio Program] TTS failed for segment', i, ':', e.message);
        seg.audioUrl = '';
        seg.status = 'tts_failed';
      }
    }
    return segments;
  }
  ...
  function generateRadioTts(text, outputPath, voice) {
    voice = voice || RADIO_TTS_VOICE;
    return new Promise(function(resolve, reject) {
      // Write text to a temp file to avoid cmd.exe Unicode corruption & shell escaping
      var textFile = outputPath.replace(/\.mp3$/, '.txt');
      var settled = false;
      try {
      ...
      function finish(err) {
        ...
        if (radioTtsQueue.length > 0) {
          var next = radioTtsQueue.shift();
          radioTtsLock = true;
          next();
        }
      ...
      function run() {
        console.log('[Radio TTS] Generating with voice:', voice, '->', outputPath);
        var proc;
        try {
          proc = spawn(RADIO_EDGE_TTS_PATH, [
            '--voice', voice,
            '-f', textFile,
            '--write-media', outputPath,
          ], { timeout: 30000, windowsHide: true });
        } catch (spawnErr) {
          console.error('[Radio TTS] spawn error:', spawnErr.message);
          finish(spawnErr);
          return;
        }
        var stderr = '';
        ...
        proc.on('close', function(code) {
          if (code !== 0) {
            console.error('[Radio TTS] exit code:', code, stderr);
            finish(new Error('edge-tts exited with code ' + code + ': ' + stderr));
            return;
          }
          try {
            if (fs.existsSync(outputPath) && fs.statSync(outputPath).size > 0) {
              console.log('[Radio TTS] Generated:', outputPath);
              settled = true;
              resolve(outputPath);
            } else {
              finish(new Error('TTS output file empty or missing'));
              return;
            }
          } catch (statErr) {
            finish(statErr);
        ...
        proc.on('error', function(err) {
          console.error('[Radio TTS] process error:', err.message);
          finish(err);
        });
      }
      if (radioTtsLock) {
        radioTtsQueue.push(run);
      } else {
        radioTtsLock = true;
        run();
      }
  ...
  function generateRadioTtsWithCache(text, outputPath, voice) {
    voice = voice || RADIO_TTS_VOICE;
    var hash = crypto.createHash('md5').update(voice + '::' + text).digest('hex');
    var cachedPath = path.join(RADIO_AUDIO_DIR, 'tts_' + hash + '.mp3');
    // Check in-memory cache
    var cached = radioTtsCache.get(hash);
    if (cached) {
      try {
        if (fs.existsSync(cached.filePath) && fs.statSync(cached.filePath).size > 0) {
          console.log('[Radio TTS] Cache HIT (mem):', hash);
          if (outputPath !== cached.filePath) fs.copyFileSync(cached.filePath, outputPath);
          cached.mtime = Date.now();
          return Promise.resolve(outputPath);
        }
      } catch(e) { radioTtsCache.delete(hash); }
    }
    // Check filesystem
    try {
      if (fs.existsSync(cachedPath) && fs.statSync(cachedPath).size > 0) {
        radioTtsCache.set(hash, { filePath: cachedPath, mtime: Date.now() });
        console.log('[Radio TTS] Cache HIT (disk):', hash);
        if (outputPath !== cachedPath) fs.copyFileSync(cachedPath, outputPath);
        pruneRadioTtsCache();
        return Promise.resolve(outputPath);
      }
    } catch(e) {}
    // Cache MISS — generate fresh
    console.log('[Radio TTS] Cache MISS, generating:', hash);
    return generateRadioTts(text, cachedPath, voice).then(function(resultPath) {
      radioTtsCache.set(hash, { filePath: cachedPath, mtime: Date.now() });
      pruneRadioTtsCache();
  ...
  function pruneRadioTtsCache() {
    if (radioTtsCache.size <= 200) return;
    var entries = Array.from(radioTtsCache.entries());
    entries.sort(function(a, b) { return a[1].mtime - b[1].mtime; });
    var toDelete = entries.slice(0, entries.length - 200);
    toDelete.forEach(function(e) { radioTtsCache.delete(e[0]); });
  }

  function cleanupOldTtsFiles() {
    try {
      var files = fs.readdirSync(RADIO_AUDIO_DIR)
        .filter(function(f) { return f.endsWith('.mp3'); })
        .map(function(f) {
          var p = path.join(RADIO_AUDIO_DIR, f);
          return { path: p, mtime: fs.statSync(p).mtimeMs };
        })
        .sort(function(a, b) { return b.mtime - a.mtime; });
      if (files.length > RADIO_MAX_AUDIO_FILES) {
        files.slice(RADIO_MAX_AUDIO_FILES).forEach(function(f) {
  ...
  // ---------- 随心电台 ----------
  if (pn === '/api/radio/intro' || pn === '/api/radio/chat') {
    try {
      var radioBody = await readRequestBody(req);
      var radioMode = radioBody.mode || (pn === '/api/radio/intro' ? 'auto_intro' : 'user_chat');
      var radioSong = radioBody.song || null;
      var radioStats = radioBody.stats || null;
      var radioMessage = radioBody.message || '';
      var radioHistory = radioBody.history || [];
      var radioPlayedSongs = radioBody.playedSongs || [];     // B
      var radioFeedback = radioBody.songFeedback || {};       // C
      var radioMemory = radioBody.memorySummary || '';        // D
      var radioVoice = radioBody.voice || '';                 // voice preference
      var radioApiKey = (radioBody.apiKey || '').trim();       // custom API key

      // Build user message for DeepSeek
      var djUserMessage = '';
      if (radioMode === 'auto_intro') {
        var summaryText = radioStats
          ? ('总播放 ' + (radioStats.totalPlays || 0) + ' 首，'
            + (radioStats.topArtist ? '最爱艺人: ' + radioStats.topArtist.name + '，' : '')
            + (radioStats.topSong ? '最爱歌曲: ' + radioStats.topSong.title + '，' : '')
            + (radioStats.recent ? '最近听过: ' + radioStats.recent.title + '，' : ''))
          : '新用户，暂无播放记录。';
        djUserMessage = '用户进入了随心电台。收听数据: ' + summaryText + ' 请做一个8-12秒的超短开场白，只需简单打个招呼+一句话点评用户的音乐口味即可，不要说太多。保持轻松自然。';
      } else if (radioMode === 'inter_song') {
        var songDesc = radioSong ? ('《' + (radioSong.title || '未知歌曲') + '》 - ' + (radioSong.artist || '未知艺人')) : '上一首歌';
        djUserMessage = '刚刚播放完了 ' + songDesc + '。请用JSON格式回复：{"comment":"12-18秒的简短评论（点评这首歌或引出下一首）","song":"推荐的下一首歌歌名","artist":"歌手名"}。注意：推荐的下一首歌绝对不能与刚刚播放的歌曲相同，必须是一首不同的歌。只输出JSON，不要额外文字。';
      } else if (radioMode === 'summarize') {
        // D: Summarize conversation for mid-term memory
        djUserMessage = '请用1-2句话总结上面这段对话中用户展现的音乐偏好。重点提炼：用户喜欢什么风格/艺人/情绪的音乐？用户不喜欢什么？用中文，保持简洁，像一个记忆便签。只输出纯文本，不要JSON。';
      } else if (radioMode === 'recommend') {
        // E: Quick song recommendation, no TTS needed
        djUserMessage = radioMessage || '请推荐一首歌';
      } else {
        djUserMessage = radioMessage || '你好';
      }

      // Build conversation messages for DeepSeek
      var djMessages = [buildRadioDjSystemPrompt({
        listenSummary: radioStats,
        playedSongs: radioPlayedSongs,
        songFeedback: radioFeedback,
        memorySummary: radioMemory,
      })];
      // A: Expand history from 6 to 20 turns
      var recentHistory = (radioHistory || []).slice(-20);
      recentHistory.forEach(function(turn) {
        if (turn.role === 'user') djMessages.push({ role: 'user', content: turn.text || turn.content || '' });
        if (turn.role === 'dj') djMessages.push({ role: 'assistant', content: turn.text || turn.content || '' });
      });
      djMessages.push({ role: 'user', content: djUserMessage });

      // Call DeepSeek API
      var djResponseText = '';
      var djMood = 'relaxed';
      try {
        var deepseekRes = await fetch(DEEPSEEK_API_URL, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': radioApiKey ? ('Bearer ' + radioApiKey) : ('Bearer ' + DEEPSEEK_API_KEY),
          },
          body: JSON.stringify({
            model: 'deepseek-chat',
            messages: djMessages,
            temperature: radioMode === 'summarize' ? 0.4 : radioMode === 'recommend' ? 0.6 : 0.85,
            max_tokens: radioMode === 'summarize' ? 150 : radioMode === 'recommend' ? 200 : 300,
          }),
        });
        if (deepseekRes.ok) {
          var dsJson = await deepseekRes.json();
          var dsContent = (dsJson.choices && dsJson.choices[0] && dsJson.choices[0].message && dsJson.choices[0].message.content) || '';
          // Try to parse JSON from response
          try {
            var parsed = JSON.parse(dsContent);
            djResponseText = parsed.text || dsContent;
            djMood = parsed.mood || 'relaxed';
          } catch (e) {
            djResponseText = dsContent;
          }
        } else {
          console.error('[Radio] DeepSeek API error:', deepseekRes.status);
          djResponseText = '抱歉，AI 服务暂时不可用（API Key 可能无效或已过期）。请在账号信息中配置你的 DeepSeek API Key，或检查服务器环境变量。';
        }
      } catch (dsErr) {
        console.error('[Radio] DeepSeek fetch error:', dsErr.message);
        djResponseText = '抱歉，AI 服务连接失败，请检查网络后重试。';
      }

      // Summarize / recommend modes: return text only, no TTS
      if (radioMode === 'summarize' || radioMode === 'recommend') {
        sendJSON(res, { text: djResponseText, audioUrl: '', mood: djMood, summary: radioMode === 'summarize' });
        return;
      }

      // Generate TTS audio
      var ttsId = 'dj_' + Date.now() + '_' + Math.random().toString(36).slice(2, 6);
      var ttsPath = path.join(RADIO_AUDIO_DIR, ttsId + '.mp3');
      try {
        await generateRadioTtsWithCache(djResponseText, ttsPath, radioVoice);
      } catch (ttsErr) {
        console.error('[Radio] TTS error:', ttsErr && ttsErr.message, ttsErr && ttsErr.stack);
        sendJSON(res, { text: djResponseText, audioUrl: '', mood: djMood, ttsError: true, ttsErrorMessage: ttsErr ? (ttsErr.message || String(ttsErr)) : 'unknown' });
        return;
      }
      ...
      sendJSON(res, {
        text: djResponseText,
        audioUrl: '/radio-audio/' + ttsId + '.mp3',
        mood: djMood,
        ttsId: ttsId,
      });
    } catch (radioErr) {
      console.error('[Radio] Endpoint error:', radioErr);
      sendJSON(res, { error: 'RADIO_ERROR', message: radioErr.message }, 500);
    }
    return;
  }
  ...
  // ---------- 随心电台 -- 持久记忆 ----------
  if (pn === '/api/radio/memory') {
    try {
      if (req.method === 'GET') {
        var mem = loadRadioMemory();
        sendJSON(res, mem);
      } else if (req.method === 'POST') {
        var memBody = await readRequestBody(req);
        var mem = loadRadioMemory();
        if (memBody.recommendedSong) {
          updateRadioMemorySong(memBody.recommendedSong);
        }
        if (typeof memBody.memorySummary === 'string') {
          // Merge: append new summary to existing
          var existing = mem.memorySummary || '';
          if (memBody.memorySummary) {
            mem.memorySummary = existing ? (existing + ' | ' + memBody.memorySummary) : memBody.memorySummary;
            // Keep it under ~500 chars
            if (mem.memorySummary.length > 500) mem.memorySummary = mem.memorySummary.slice(-500);
          }
          saveRadioMemory(mem);
        }
        if (memBody.reset) {
          mem.recommendedSongs = [];
          mem.memorySummary = '';
          mem.userTaste = '';
          saveRadioMemory(mem);
        }
      ...
      console.error('[Radio Mem] Endpoint error:', memErr);
      sendJSON(res, { error: 'MEM_ERROR', message: memErr.message }, 500);
    }
    return;
  }

  // ---------- 随心电台 -- 节目编排 ----------
  if (pn === '/api/radio/program') {
    try {
      var radioProgBody = await readRequestBody(req);
      var radioProgStats = radioProgBody.stats || null;
      var radioProgHistory = radioProgBody.history || [];
      var radioProgPlayedSongs = radioProgBody.playedSongs || [];
      var radioProgFeedback = radioProgBody.songFeedback || {};
      var radioProgMemory = radioProgBody.memorySummary || '';
      var radioProgVoice = radioProgBody.voice || '';
      var radioProgRecentSongs = radioProgBody.recentSongs || [];
      var radioProgMessage = radioProgBody.message || '';
      var radioProgIsFirstVisit = radioProgBody.isFirstVisit === true;
      var radioProgApiKey = (radioProgBody.apiKey || '').trim();

      // Build program prompt with user data
      var djProgMessages = [buildRadioDjProgramPrompt({
        isFirstVisit: radioProgIsFirstVisit,
        listenSummary: radioProgStats,
        playedSongs: radioProgPlayedSongs,
        songFeedback: radioProgFeedback,
        memorySummary: radioProgMemory,
        recentSongs: radioProgRecentSongs,
      })];
      var recentProgHistory = (radioProgHistory || []).slice(-10);
      recentProgHistory.forEach(function(turn) {
        if (turn.role === 'user') djProgMessages.push({ role: 'user', content: turn.text || turn.content || '' });
        if (turn.role === 'dj') djProgMessages.push({ role: 'assistant', content: turn.text || turn.content || '' });
      });
      var djProgUserMsg = radioProgMessage
        ? '用户的请求：' + radioProgMessage + '\n请根据这个请求编排一期电台节目。'
        : '请编排一期新的电台节目。根据当前时间、季节、用户偏好来设计节目主题和选曲。';
      djProgMessages.push({ role: 'user', content: djProgUserMsg });

      // Call DeepSeek to generate program
      var program;
      try {
        var dsRes = await fetch(DEEPSEEK_API_URL, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': radioProgApiKey ? ('Bearer ' + radioProgApiKey) : ('Bearer ' + DEEPSEEK_API_KEY) },
          body: JSON.stringify({ model: 'deepseek-chat', messages: djProgMessages, temperature: 0.85, max_tokens: 600 }),
        });
        if (dsRes.ok) {
          var dsJson = await dsRes.json();
          var dsContent = (dsJson.choices && dsJson.choices[0] && dsJson.choices[0].message && dsJson.choices[0].message.content) || '';
          try {
            program = JSON.parse(dsContent);
          } catch (e) {
            // Try to extract JSON from markdown
            var m = dsContent.match(/\{[\s\S]*\}/);
            if (m) { try { program = JSON.parse(m[0]); } catch (e2) {} }
            if (!program) {
              console.error('[Radio Program] Failed to parse AI response:', dsContent.slice(0, 200));
              sendJSON(res, { error: 'PARSE_ERROR', message: 'AI response was not valid JSON', raw: dsContent.slice(0, 300) }, 500);
              return;
            }
          }
        } else {
          console.error('[Radio Program] DeepSeek API error:', dsRes.status);
          sendJSON(res, { error: 'AI_ERROR', message: 'DeepSeek API returned ' + dsRes.status }, 500);
          return;
        }
      } catch (e) {
        console.error('[Radio Program] Fetch error:', e.message);
        sendJSON(res, { error: 'FETCH_ERROR', message: e.message }, 500);
        return;
      }

      // Validate program structure
      var programTitle = program.title || '随心电台';
      var programMood = program.mood || 'relaxed';
      var programTracks = program.tracks || [];
      var programSegments = program.segments || [];

      if (!programTracks.length) {
        sendJSON(res, { error: 'EMPTY_PROGRAM', message: 'AI returned no tracks' }, 500);
        return;
      }
      if (programTracks.length > 4) {
        programTracks = programTracks.slice(0, 4);
      }

      // Resolve tracks to playable URLs
      console.log('[Radio Program] Resolving', programTracks.length, 'tracks...');
      var trackResult = await resolveRadioTracks(programTracks);
      var resolvedTracks = trackResult.resolved;

      if (!resolvedTracks.length) {
        sendJSON(res, { error: 'NO_RESOLVED_TRACKS', message: 'Could not resolve any tracks', failedTracks: trackResult.failed }, 500);
        return;
      }

      // Synthesize TTS for all segments
      console.log('[Radio Program] Synthesizing TTS for', programSegments.length, 'segments...');
      await synthesizeRadioSegments(programSegments, radioProgVoice);

      cleanupOldTtsFiles();

      // Record recommended songs to persistent memory
      resolvedTracks.forEach(function(t) {
        updateRadioMemorySong({ title: t.title || t.name, artist: t.artist, action: 'recommended' });
      });
      ...
      sendJSON(res, {
        ...
        failedTracks: trackResult.failed,
      });
    } catch (e) {
      console.error('[Radio Program] Error:', e);
      sendJSON(res, { error: 'PROGRAM_ERROR', message: e.message }, 500);
    }
    return;
  }
